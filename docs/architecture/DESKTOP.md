...
- **Preload**: exposes `window.autoappz = { dispatch, subscribe, startStream, cancel, version }`. No `ipcRenderer` passthrough, no channel strings from the renderer; channel names are derived from a generated registry compiled into the preload.
...
- Trusted-frame check on every command: sender frame is the main frame of a registered window and its URL is the packaged renderer or dev origin.
