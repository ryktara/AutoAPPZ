...
describe("migrations", () => {
  ...
  it("is idempotent across reopen and backs up before applying new migrations", async () => {
    await withTempDir(async (dir) => {
      ...
      const extra: Migration = { id: "0005_extra", up: ["CREATE TABLE extra (id TEXT PRIMARY KEY)"] };
      const h3 = openDatabase({ path: file, migrations: [...ALL_MIGRATIONS, extra] });
      expect(h3.appliedMigrations).toEqual(["0005_extra"]);
      h3.close();
      const backups = readdirSync(path.join(dir, "backups"));
      expect(backups).toHaveLength(1);
      expect(backups[0]).toMatch(/pre-0005_extra\.db$/);
  ...
  it("restores the backup when a migration fails", async () => {
    await withTempDir(async (dir) => {
      ...
      const broken: Migration = { id: "0005_broken", up: ["CREATE TABLE ok (id TEXT)", "THIS IS NOT SQL"] };
